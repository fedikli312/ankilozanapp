...
**Update (Phase Design-B implementation):** the token/component foundation and navigation shell described in principle below are now implemented — `docs/DESIGN_SYSTEM_2_0_IMPLEMENTATION.md` is the authoritative record of the real, verified values and is the one to read for anything code-facing. Three notes where implementation diverged from this document's proposal, beyond the already-superseded §1/§3 above:
- **§4 (App icon concept)** is superseded by the implemented app-icon foundation (`assets/brand/app-icon-source.svg`) — a flat, single-color Margin Mark on a solid `brandPrimary` field, not the gradient "Held Line" stroke described below (which followed the now-superseded §3 logo). See the implementation doc §9 for the finished vector and what raster-export work remains.
- **§5 (Color direction)** — the hex values below were a starting proposal; several failed real WCAG contrast verification and were adjusted (most notably the dusty-gold accent, measured at 2.72:1 as text and darkened to 4.68:1). Treat the implemented, verified values in the implementation doc §1 as authoritative, not the hexes below.
- **§19 (Navigation)** is implemented as proposed and confirmed (3 visible tabs: Today / Health Record / Appointments) — see the implementation doc §10 for exactly what shipped (Health Record is a relabel of the existing Track route, not a new screen; Insights is hidden from the tab bar via `href: null` but not deleted).

Everything else in this document (brand character, typography/layout/component philosophy, per-screen philosophies) remains a **proposal** — none of the actual product screens it describes (Today, Check-in, Track/Health Record content, Timeline, Appointment screens, Insights, onboarding, paywall) were redesigned in Design-B.
